...
| `/rdo/relatorios` | `rdo.relatorios.read` | `projectId`, `reportType`, `status`, `reportDate`, `arrivalTime`, `departureTime`, `lunchBreak`, `daytimeCount`, `daytimeWorkedMinutes`, `nighttimeWorkedMinutes`, `daytimeOvertimeMinutes`, `nighttimeOvertimeMinutes`, `totalOvertimeMinutes`, `approvedAt` | Somente relatórios aprovados, não excluídos e de projetos autorizados não excluídos. |
...
- `projectId` somente nos recursos com vínculo a projeto (inclusive colaboradores); deve pertencer aos projetos do token. Cadastros globais não aceitam esse filtro. `active` somente nos recursos com `isActive`.
...
- Escopo não implica acesso a relações, contatos, observações, JSON livre, arquivos, custos ou dados pessoais completos. Cada consulta usa `select` explícito e a resposta é serializada pela mesma allowlist tipada.
