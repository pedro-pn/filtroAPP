...
Envelope e cursor assinado compatíveis com as coleções existentes. `limit`, `cursor`, `snapshotAt` em todas; `projectCode` (recomendado) ou `projectId` (compatibilidade) onde há vínculo; `reportType` e `reportId` nas coleções ligadas a relatórios; `maintenanceId` ou `itemId` somente nas coleções correspondentes. Os filtros de projeto são mutuamente exclusivos e respeitam o recorte do token. `reportType` aceita um ou mais códigos separados por vírgula. `updatedSince` apenas onde existe `updatedAt`; registros com apenas `createdAt` aceitam `createdSince` e ordenam `(createdAt,id)`. Equipe ordena `(reportId,collaboratorId)`, sem data inventada e sem filtro incremental. O teto de equipe aplica-se à criação do relatório pai.
