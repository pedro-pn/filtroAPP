...
if [ -z "$BACKUP_SOURCE" ]; then
  echo "[restore] set BACKUP_SOURCE to the directory containing postgres.sql.gz and relatorios.tar.gz" >&2
  exit 1
fi
...
if [ ! -f "$BACKUP_SOURCE/relatorios.tar.gz" ]; then
  echo "[restore] file not found: $BACKUP_SOURCE/relatorios.tar.gz" >&2
  exit 1
fi
