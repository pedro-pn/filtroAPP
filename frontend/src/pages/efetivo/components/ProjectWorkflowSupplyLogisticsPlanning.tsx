    ...
    {editing ? <div className="project-workflow-resource-editor">
      ...
      {draft.length ? <div className="project-workflow-supply-draft">{draft.map(item => {
        ...
        return <article className={purchaseRequired ? 'has-warning' : 'is-ready'} key={item.id}>
          ...
          {purchaseRequired ? <div className="project-workflow-purchase-tracking"><p>⚠ {stockItem ? `Faltam ${shortage} ${item.unitLabel} após considerar as reservas existentes.` : 'Este item precisa ser adquirido e cadastrado.'}</p><div><label className="project-workflow-release-toggle"><input type="checkbox" checked={Boolean(item.requestedAt)} disabled={saving} onChange={event => updateItem(item.id, event.target.checked ? { requestedAt: todayDateOnly() } : { requestedAt: null, purchasedAt: null })} /><span>Pedido realizado{item.requestedAt ? ` em ${displayDateOnly(item.requestedAt)}` : ''}</span></label><label className="project-workflow-release-toggle"><input type="checkbox" checked={Boolean(item.purchasedAt)} disabled={saving || !item.requestedAt} onChange={event => updateItem(item.id, { purchasedAt: event.target.checked ? todayDateOnly() : null })} /><span>Compra concluída{item.purchasedAt ? ` em ${displayDateOnly(item.purchasedAt)}` : ''}</span></label></div>{stockItem?.reservationConflicts.length ? <div className="field-group"><label htmlFor={`workflow-supply-exception-${item.id}`}>Justificativa para manter a reserva</label><textarea id={`workflow-supply-exception-${item.id}`} rows={2} maxLength={1000} value={item.reservationExceptionReason || ''} disabled={saving} placeholder="Registre o motivo da exceção, se aplicável." onChange={event => updateItem(item.id, { reservationExceptionReason: event.target.value || null })} /></div> : null}{invalidDates ? <small>A compra deve ser registrada depois da solicitação.</small> : null}</div> : <p className="project-workflow-stock-ok">✓ Saldo disponível suficiente para a quantidade planejada.</p>}
        </article>;
