  ...
  >
    ...
    <section className="project-workflow-logistics-section">
      ...
      {draft.lodgingRequired === true ? <div className="project-workflow-logistics-details">
        <div className="project-workflow-logistics-fields"><div className="field-group"><label htmlFor="workflow-logistics-lodging-people">Pessoas</label><input id="workflow-logistics-lodging-people" type="number" min="1" max="1000" step="1" value={draft.lodgingPeopleCount ?? ''} disabled={!canEdit} onChange={event => change('lodgingPeopleCount', event.target.value ? Number(event.target.value) : null)} onBlur={() => save(draft)} /></div><div className="field-group"><label htmlFor="workflow-logistics-lodging-date">Data prevista</label><input id="workflow-logistics-lodging-date" type="date" value={draft.lodgingExpectedDate || ''} disabled={!canEdit} onChange={event => save({ ...draft, lodgingExpectedDate: event.target.value || null })} /></div></div>
        <div className="project-workflow-resource-question"><div><strong>A hospedagem já foi solicitada?</strong><p>Registre as datas para acompanhar a solicitação até a conclusão.</p></div><ProjectWorkflowBooleanChoice value={draft.lodgingRequested} label="Hospedagem solicitada" disabled={!canEdit} onSelect={value => save({ ...draft, lodgingRequested: value, lodgingRequestedAt: value ? draft.lodgingRequestedAt : null, lodgingCompletedAt: value ? draft.lodgingCompletedAt : null })} /></div>
        {draft.lodgingRequested === true ? <div className="project-workflow-logistics-fields"><div className="field-group"><label htmlFor="workflow-logistics-lodging-requested">Data da solicitação</label><input id="workflow-logistics-lodging-requested" type="date" value={draft.lodgingRequestedAt || ''} disabled={!canEdit} onChange={event => save({ ...draft, lodgingRequestedAt: event.target.value || null, lodgingCompletedAt: event.target.value && draft.lodgingCompletedAt && draft.lodgingCompletedAt < event.target.value ? null : draft.lodgingCompletedAt })} /></div><div className="field-group"><label htmlFor="workflow-logistics-lodging-completed">Data da conclusão</label><input id="workflow-logistics-lodging-completed" type="date" min={draft.lodgingRequestedAt || undefined} value={draft.lodgingCompletedAt || ''} disabled={!canEdit || !draft.lodgingRequestedAt} onChange={event => save({ ...draft, lodgingCompletedAt: event.target.value || null })} /></div></div> : null}
      </div> : null}
    </section>
