...
function DocumentationTypeCard({ category, workflow, saving, onPatch }: {
  ...
  return (
    <article className={`project-workflow-documentation-type is-${category.required === true ? 'required' : category.required === false ? 'not-required' : 'unanswered'}`}>
      <header><div><h5>{category.label}</h5><p>Este tipo é necessário para o projeto?</p></div><div className="project-workflow-documentation-choice"><Button type="button" variant={category.required === true ? 'primary' : 'secondary'} disabled={saving || !workflow.permissions.canEdit} onClick={() => onPatch({ action: 'documentation_category', version: workflow.version, type: category.type, required: true })}>Sim</Button><Button type="button" variant={category.required === false ? 'primary' : 'secondary'} disabled={saving || !workflow.permissions.canEdit} onClick={() => onPatch({ action: 'documentation_category', version: workflow.version, type: category.type, required: false })}>Não</Button></div></header>
