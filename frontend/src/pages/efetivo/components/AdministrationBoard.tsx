...
function NotificationEmailRow({ setting, canManage, onSaved }: { setting: NotificationEmailSetting; canManage: boolean; onSaved: () => void }) {
  ...
  return <article className="efetivo-admin-role">
    <div><strong>{setting.label}</strong><p className="placeholder-copy">{setting.description}</p></div>
    <div className={`field-group ${invalid ? 'field-invalid' : ''}`}>
      <label htmlFor={`notification-email-${setting.key}`}>E-mail de aviso</label>
      <input id={`notification-email-${setting.key}`} type="email" value={email} disabled={!canManage} aria-invalid={invalid} placeholder="nome@filtrovali.com.br" onChange={event => setEmail(event.target.value)} />
      {invalid ? <span className="field-error">Informe um e-mail válido.</span> : null}
    </div>
    {canManage ? <Button variant="mini" disabled={mutation.isPending || invalid || email.trim() === ''} onClick={() => mutation.mutate()}>Salvar</Button> : null}
  </article>;
}

export function AdministrationBoard({ canManage, tab, onTabChange }: {
  canManage: boolean;
  tab: 'regras' | 'feriados' | 'notificacoes' | 'atividade';
  onTabChange: (tab: 'regras' | 'feriados' | 'notificacoes' | 'atividade') => void;
}) {
  ...
  return <div className="efetivo-board" data-efetivo-administration>
    <nav className="page-card efetivo-admin-tabs" aria-label="Áreas da administração">{([['regras', 'Regras e acessos'], ['feriados', 'Feriados'], ['notificacoes', 'Notificações'], ['atividade', 'Atividade']] as const).map(([value, label]) => <button type="button" className={tab === value ? 'active' : ''} aria-current={tab === value ? 'page' : undefined} onClick={() => onTabChange(value)} key={value}>{label}</button>)}</nav>
    {tab === 'regras' ? <>
    ...
    {tab === 'notificacoes' ? <section className="page-card"><div className="efetivo-section-heading"><div><h2>E-mails de aviso</h2><p>Destinatário padrão de cada aviso automático da Gestão de Projetos. Pode ser corrigido pontualmente em cada projeto.</p></div></div>{notificationEmails.isLoading ? <p className="placeholder-copy">Carregando e-mails…</p> : notificationEmails.isError ? <p className="placeholder-copy">Não foi possível carregar os e-mails.</p> : <div className="efetivo-admin-role-list">{notificationEmails.data?.map(setting => <NotificationEmailRow setting={setting} canManage={canManage} onSaved={() => queryClient.invalidateQueries({ queryKey: ['efetivo-notification-emails'] })} key={setting.key} />)}</div>}</section> : null}
