...
export function HistoricalServicesContent({ projects, projectId, onProjectChange, onBusyChange }: {
  ...
  return <div className="historical-content">
    ...
    <p className="historical-help">Use RLQ para limpeza química, RTP para teste de pressão e RCPU para filtragem ou flushing. Equipamento do cliente e sistema são textos livres. Diâmetros: 2 pol, 2' ou 50 mm (sem unidade = polegadas). Comprimentos: cm ou m. Volumes: L ou mL. Sistemas completos: UN, com quantidade inteira e diâmetro vazio (somente limpeza química). Substitua os exemplos do modelo pelos dados dos relatórios.</p>
      ...
      <fieldset disabled={busy}>
        ...
        {form.items.map((item, index) => <div className="historical-item-editor" key={index}>
          <div className="historical-form-grid">
            ...
            <div className="field-group"><label htmlFor={`hs-diameter-${index}`}>Diâmetro (pol ou mm)</label><input id={`hs-diameter-${index}`} disabled={['L', 'mL', 'UN'].includes(item.unit)} required={['m', 'cm'].includes(item.unit)} placeholder={['m', 'cm'].includes(item.unit) ? 'Ex.: 2 pol, 1 1/2 pol ou 50 mm' : 'Não se aplica'} value={item.diameter} onChange={event => updateItem(index, { diameter: event.target.value })} /></div>
            ...
            <div className="field-group"><label htmlFor={`hs-unit-${index}`}>Unidade</label><select id={`hs-unit-${index}`} value={item.unit} onChange={event => updateItem(index, { unit: event.target.value as HistoricalMeasurement['unit'], diameter: ['L', 'mL', 'UN'].includes(event.target.value) ? '' : item.diameter })}>
              {item.serviceType === 'limpeza' && <option value="UN">un — unidades (sistemas completos)</option>}
