...
import { Card, Button, Badge, Alert, Field, Input, Textarea, Skeleton, EmptyState } from '../ui/ds';
...
export function ProjectDetailDashboard({
  ...
  const { data: qualityDeviations = [], isLoading: qualityDeviationsLoading, isError: qualityDeviationsError, refetch: refetchQuality } = useQuery<ProjectDeviation[]>({
    ...
    <div className="fv-ds acp-detail">
      ...
      <div className="acp-detail-cols">
        ...
        <div className="acp-detail-col">
          ...
          <Card padding="sm" className="acp-detail-block">
            <MetricBar label={`Avanço do escopo${progressSuffix}`} value={data.avancoPct} caption={fmtPct(data.avancoPct)}
              help="Quanto do escopo vendido já foi executado: cruza o realizado dos RDOs (metros de tubulação, litros de óleo) com o previsto, ponderado pelo peso de cada serviço. Sem escopo cadastrado, usa o avanço manual informado no cronograma." />
      ...
      {!isGroup ? (
        <Card padding="sm" className="acp-detail-block acp-detail-deviations" data-quality-project-deviations>
          ...
          {qualityDeviationsError ? (
            <Alert tone="warning" title="Não foi possível carregar os desvios."
              action={<Button size="sm" variant="secondary" onClick={() => void refetchQuality()}>Tentar novamente</Button>} />
      ...
      <ProjectDetailPeople data={data} isGroup={isGroup} onSelect={(collaborator, source) => setHoursDetail({ collaborator, source })} />
