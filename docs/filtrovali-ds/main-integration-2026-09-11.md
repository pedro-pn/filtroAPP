...
| TAG e romaneios — #238/#239 | `ProjectOverviewMetrics`, `ProjectDetailDashboard`, `ProjectRomaneiosDialog` | TAG/código ao lado do nome nos cards/detalhe; diálogo de todos os itens de entrada/saída, projeto/grupo, quantidades/unidades; consulta não exige papel de edição do Romaneio | TAG preservada na conciliação; **A4 ampliado** para migrar diálogo e gatilho |
...
- [ ] **A3a.1 (P1, próximo)** — migrar faturamentos do detalhe e conferir TAG nos
  cards/grupos. Cobrir todos os estados de sincronização e distinguir bruto/recebido.
