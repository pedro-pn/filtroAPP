...
const rliTemplatePath = path.resolve(__dirname, '../../../Modelos/definitivos/Modelo - RLI.docx');
...
function splitSystemValue(value) {
  const [system = '', description = '', diagram = ''] = stringify(value).split(';').map(part => part.trim());
  return { system, description, diagram };
}
...
function embedPhotos(zip, doc, placeholder, assets) {
  ...
  for (let i = 0; i < assets.length; i += 2) {
    ...
    group.forEach((asset, idx) => {
      const relId = addImageRel(zip, relsDoc, asset, 'rli-photo');
...
function embedSignature(zip, doc, asset) {
  ...
  const relsDoc = new DOMParser().parseFromString(zip.readAsText(relsEntry), 'text/xml');
  const relId = nextRelId(relsDoc);
  const mediaName = `signature-rli-${Date.now()}.${asset.extension}`;
  zip.addFile(`word/media/${mediaName}`, asset.bytes);
  ensureContentType(zip, asset.extension, asset.mimeType);
  const rel = relsDoc.createElement('Relationship');
  rel.setAttribute('Id', relId);
...
function buildRliBaseData(report) {
  ...
  const systemParts = splitSystemValue(getField(sd, ['Sistema']));
  ...
  return {
    ...
    rli: reportNumber(report),
    date: formatDatePt(report.reportDate),
    equipament: stringify(getField(sd, ['Embarcação', 'Embarcacao', 'ID da embarcação', 'ID da embarcacao', 'Equipamento(s)', 'Equipamento'])),
    system: systemParts.system,
    description: systemParts.description,
    diagram: systemParts.diagram,
    material: stringify(getField(sd, ['Material da tubulação', 'Material da tubulacao'])),
...
function expandRliCollaborators(doc, collaborators) {
...
`;
  const scriptPath = path.join(env.uploadDir, `tmp-rli-pdf-${Date.now()}.ps1`);
...
export async function organizeRliPhotos(report, projectFolderName) {
  ...
  const photosDir = path.join(env.uploadDir, projectFolderName, 'Registros Fotográficos', 'RLI');
  ...
  for (const upload of cleaningUploads) {
    ...
    try {
      ...
      const newUrl = buildPhotoUrl(extractUrlBase(source), projectFolderName, 'RLI', destName);
...
export async function buildRliDocx(report) {
  const sc = report.specialConditions || {};
  const sd = sc.serviceData || {};
  ...
  const bytes = await fs.readFile(rliTemplatePath);
  ...
  updateXmlEntry(zip, 'word/document.xml', doc => {
    ...
    expandRliCollaborators(doc, collabs);
    embedPhotos(zip, doc, '{{platephotos}}', cleaningAssets);
...
export async function saveRliDocx(report) {
  const bytes = await buildRliDocx(report);
  const projectFolderName = safePath(`Missão ${report.project.code} - ${report.project.name}`);
  const dir = path.join(env.uploadDir, projectFolderName, 'RLI');
  ...
  return {
    fileName,
    targetPath,
    publicUrl: `/relatorios/${encodeURIComponent(projectFolderName)}/RLI/${encodeURIComponent(fileName)}`
  };
}

export async function saveRliPdf(report) {
  const docx = await saveRliDocx(report);
