...
const rlfTemplatePath = path.resolve(__dirname, '../../../Modelos/definitivos/Modelo - RLF.docx');
...
function embedPhotos(zip, doc, placeholder, assets) {
  ...
  for (let i = 0; i < assets.length; i += 2) {
    ...
    group.forEach((asset, idx) => {
      const relId = addImageRel(zip, relsDoc, asset, 'rlf-photo');
...
function embedSignature(zip, doc, asset) {
  ...
  const relsDoc = new DOMParser().parseFromString(zip.readAsText(relsEntry), 'text/xml');
  const relId = nextRelId(relsDoc);
  const mediaName = `signature-rlf-${Date.now()}.${asset.extension}`;
  zip.addFile(`word/media/${mediaName}`, asset.bytes);
  ensureContentType(zip, asset.extension, asset.mimeType);
  const rel = relsDoc.createElement('Relationship');
  rel.setAttribute('Id', relId);
...
function buildRlfBaseData(report) {
...
function expandRlfCollaborators(doc, collaborators) {
...
`;
  const scriptPath = path.join(env.uploadDir, `tmp-rlf-pdf-${Date.now()}.ps1`);
...
export async function buildRlfDocx(report) {
  ...
  const bytes = await fs.readFile(rlfTemplatePath);
  ...
  updateXmlEntry(zip, 'word/document.xml', doc => {
    ...
    expandRlfCollaborators(doc, collabs);
    embedPhotos(zip, doc, '{{filterphotos}}', cleaningAssets);
...
export async function saveRlfDocx(report) {
  const bytes = await buildRlfDocx(report);
  const sc = report.specialConditions || {};
  const sd = sc.serviceData || {};
  const projectFolderName = safePath(`Missão ${report.project.code} - ${report.project.name}`);
  const dir = path.join(env.uploadDir, projectFolderName, 'RLF');
  ...
  return {
    fileName,
    targetPath,
    publicUrl: `/relatorios/${encodeURIComponent(projectFolderName)}/RLF/${encodeURIComponent(fileName)}`
  };
}

export async function saveRlfPdf(report) {
  const docx = await saveRlfDocx(report);
