...
test('historical import HTTP + PostgreSQL: permissions, preview, atomic writes, reimport, editing and progress', {
  ...
  assert.deepEqual(imported.data, { created: 4, skipped: 0 });
  assert.deepEqual((await request(`/${project.id}/import`, 'POST', importBody)).data, { created: 0, skipped: 4 });
  ...
  const rlq = listed.data.items.find(report => report.reportType === 'RLQ');
  const editedCsv = HISTORICAL_CSV_TEMPLATE.split('\r\n').filter(line => line.includes('Relatorio') || line.startsWith('RLQ')).join('\n').replace(';35;m', ';36;m');
