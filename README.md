...
- **Faturamentos realizados**: o dashboard de cada projeto e grupo de missões lista NFS-e faturadas e NF-e de venda, com valor bruto, tomador e situação de recebimento. O histórico fiscal completo fica em `OmieInvoice`, separado dos títulos incrementais; parcelas não duplicam notas, e cancelamentos, remessas e notas de débito não entram no total. A atualização ocorre no ciclo Omie; falhas preservam a última consulta concluída. Após aplicar as migrations e gerar o Prisma Client, `node scripts/omie-sync.js faturamentos` (em `backend`) permite carregar somente esse histórico. A consulta usa o vínculo atual dos projetos Omie e não altera os cálculos gerenciais existentes de receita/impostos.
